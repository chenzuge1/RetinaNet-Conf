...
class AnchorHead(BaseDenseHead):
    ...
    def _get_targets_single(self,
        ...
        if unmap_outputs:
            ...
            assigned_gt_inds = unmap(assigned_gt_inds, num_total_anchors, inside_flags)
    ...
    def get_targets(self,
        ...
        # (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights, all_assigned_gt_inds, gt_bboxes,
        ...
        # assigned_gt_inds_list = images_to_levels(all_assigned_gt_inds,
        #                                      num_level_anchors)
        # gt_bboxes =
        res = (labels_list, label_weights_list, bbox_targets_list,
               bbox_weights_list, num_total_pos, num_total_neg)
    ...
    def loss_single(self, cls_score, bbox_pred, iou_pred, anchors, labels, label_weights,
        ...
        iou_target = bbox_overlaps(bbox_preds, bbox_targets, is_aligned=True)
        iou_target = iou_target.reshape(-1)

        iou_pred = iou_pred.permute(0, 2, 3, 1).reshape(-1)
        bbox_weight_list = torch.split(bbox_weights, 1, -1)
        bbox_weight = bbox_weight_list[0].squeeze()

        iou_target[bbox_weight == 0] = 0
        ...
        # FocalLoss
        # iou_label = torch.clamp((labels.reshape(-1) + 1) % 81, max=1.0)   # label中 80代表背景，0-79表示类别，因此iou_label用0表示背景，1表示对象
        # iou_pred = iou_pred.permute(0, 2, 3,1).reshape(-1, 1)
        # loss_iou = self.loss_iou(
        #     iou_pred, iou_label, label_weights, avg_factor=num_total_samples)
    ...
    def loss(self,
        ...
        Args:
            ...
            bbox_preds (list[Tensor]): Box energies / deltas for each scale
                level with shape (N, num_anchors * 4, H, W)
            bbox_ious (list[Tensor]): Box iou for each scale
    ...
    def get_bboxes(self,
        ...
        assert len(cls_scores) == len(iou_preds)
    ...
    def _get_bboxes_single(self,
        ...
        assert len(cls_score_list) == len(bbox_pred_list) == len(mlvl_anchors) == len(iou_pred_list)
        mlvl_bboxes = []
        mlvl_scores = []
        for cls_score, bbox_pred, iou_pred, anchors in zip(cls_score_list,
                                                           bbox_pred_list, iou_pred_list, mlvl_anchors):
            ...
            # alpha = 0.6
            alpha = 0.4
            scores = torch.pow(scores, 1-alpha)
            iou_pred = torch.pow(iou_pred, alpha)
